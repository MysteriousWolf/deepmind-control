...
**Nothing about the instrument is written down in this tree.** The front panel's
contents were a table in `control-ui/src/home.rs` until 26.3 published them. The
shapes the plate displays draw were arithmetic in `control-ui/src/scene.rs` and
`envelope.rs` until 26.4 published them. The last transcribed number was
`HIGH_PASS_SLOPE`, 6 dB per octave, copied out of a library doc comment; 26.5
published the curve it belonged to and it is deleted.
