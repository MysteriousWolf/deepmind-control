...
| [#24](https://github.com/MysteriousWolf/deepmind-midi/issues/24) | A sequencer step is bipolar, `0` skips it, and `Sequence Length` bounds which steps play | The strip gains a centre line, a skip mark and a dimmed tail, and the sequencer becomes a display the arpeggiator's plate could carry beside its gates. Also where the `kind = "switch"` on two steps that accept 256 values gets settled |
| [#25](https://github.com/MysteriousWolf/deepmind-midi/issues/25) | A parameter's displayed range, as the manual prints it | A fader could say what its two ends mean, the way every effect slot already does, and a display's axis could carry the two ends of the range it is drawing across. 26 parameters have one. It is not a curve, so a corner would still sit where the byte sits in its own travel |
...
| [#28](https://github.com/MysteriousWolf/deepmind-midi/issues/28) | What a parameter *does* — the manual's sentence, behind a default-off feature | The footer under the window describes whatever the pointer is over out of five things the library already answers, and the sixth is the one somebody pointed at the control to ask. Deliberately absent rather than missing: the library keeps the prose out of the binary for the targets it is built for, so the ask is a feature and an `Option`, not a reversal |
