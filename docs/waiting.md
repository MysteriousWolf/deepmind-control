...
| --- | --- | --- |
| [#38](https://github.com/MysteriousWolf/deepmind-midi/issues/38) | What a modulation depth is worth: the law between `Mod n Depth` and the destination's own range | **Assumes full depth moves the destination over its whole range.** A routing pointed at the window is set by taking hold of the control it should move, and how far the drag went is the depth — which needs the fraction of a destination's range a full depth covers, and nothing publishes it. The assumption is marked in `control-ui/src/mapping.rs` and printed on the page itself. It blocks nothing: the drag puts a number in the same byte the depth fader already held, and the fader is unchanged |
| [#40](https://github.com/MysteriousWolf/deepmind-midi/issues/40) | A dot-matrix cell for every modulation source and destination | Draws the patch bay beside the eight rows with the **names** in each column rather than a picture. Sources down one side, destinations down the other, a line for each routing between them — which is the one thing the rows cannot show, and which would be twice as quick to read as two columns of seven-by-seven cells. The cells are the library's for the reason the effect families' marks were: a picture of `LFO 1` is a fact about the instrument, and one drawn here would be this window inventing one |
| [#41](https://github.com/MysteriousWolf/deepmind-midi/issues/41) | Which way a modulation source swings a control it reaches | **Assumes the depth's own sign is the whole answer.** While a routing is being mapped, every control the other routings already reach carries a band showing how far each can push it, and the band runs from where the control sits in the direction the depth's sign gives. That is right for a wheel and for an envelope, and wrong for an LFO, which swings a control about where it sits rather than up from it. Marked in `control-ui/src/mapping.rs` and said out loud on the page. The amount is [#38](https://github.com/MysteriousWolf/deepmind-midi/issues/38); this is the direction, and they are two facts on two parameters |
