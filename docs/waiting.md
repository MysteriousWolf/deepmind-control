...
What is left is `SILKSCREEN`, four rows of it, and #45 is the ask that empties
it. It is a smaller kind of transcription than the one 26.3 replaced — it names
...
| --- | --- | --- |
| [#45](https://github.com/MysteriousWolf/deepmind-midi/issues/45) | The presses and the printed rules `front::sections()` does not carry | **Transcribes four of them, in a table written to delete itself.** The instrument has three buttons along a section's foot whose parameters the table already has and whose `PanelControl` it does not — `OSC 1 Saw Enable`, `OSC 1 Pulse Enable` and `VCF Envelope Polarity`, which the panel prints as a sawtooth, a pulse and `INVERT` — and it rules a thin line between the clusters of faders inside a wide section, which `Section` has no way to say. `SILKSCREEN` in `control-ui/src/home.rs` is those four facts and nothing else: it names no parameter the library lacks, only where on the front one is printed. A press the library starts carrying is skipped rather than drawn twice, and `the_silkscreen_table_is_still_needed` fails on the row that has become dead weight, so the release that answers this is a red build here rather than a quiet duplicate. The interesting half is the two waveform presses: their legend is a wave and not a word, and `PanelControl::legend` is a `&str`, so the table cannot say so even if it listed them |
